...
with tab_stock:
    ...
    st.markdown("Identifies periods where high-sentiment tweets coincide with unusual stock volatility relative to the 14-day baseline.")
                            ...
                            if stocks_df.empty:
                                ...
                                with atr_results_container:
                                    ...
                                    if results_df.empty:
                                        ...
                                        st.subheader("Next-Day Price Swing vs Average (14-day)")
                                        st.bar_chart(viz_df)
                                        ...
                                        st.write(f"**{exceeded}/{total}** tweet days had next-day range above Average. "
...
with tab_stock:
    ...
    with ctrl1:
        ...
        show_sma_20 = st.checkbox("Simple Average 20", value=True)
        show_sma_50 = st.checkbox("Simple Average 50", value=True)
        show_sma_200 = st.checkbox("Simple Average 200")
        show_ema_12 = st.checkbox("Recent Average 12")
        show_ema_26 = st.checkbox("Recent Average 26")
    with ctrl2:
        st.markdown("**Price Bounds**")
        show_bb = st.checkbox("Show Price Bounds", value=True)
        bb_period = st.slider("Bounds Period", 5, 50, 20, disabled=not show_bb)
        bb_std_val = st.slider("Bounds Width", 1.0, 3.0, 2.0, 0.5, disabled=not show_bb)
    with ctrl3:
        st.markdown("**Indicators**")
        show_rsi = st.checkbox("Momentum", value=True)
        rsi_period = st.slider("Momentum Period", 5, 30, 14, disabled=not show_rsi)
        show_macd = st.checkbox("Trend Shift", value=True)
                        ...
                        if display_df.empty:
                            ...
                            with stock_chart_container:
                                ...
                                if last_rsi is not None:
                                    rsi_label = "Too High" if last_rsi > 70 else ("Too Low" if last_rsi < 30 else "Neutral")
                                    m5.metric("Momentum", f"{last_rsi:.1f} — {rsi_label}")
                                else:
                                ...
                                if show_rsi:
                                    ...
                                    subplot_titles_chart.append(f"Momentum ({rsi_period})")
                                if show_macd:
                                    macd_row_num = n_rows_chart + 1
                                    n_rows_chart += 1
                                    row_heights_chart.append(0.125)
                                    subplot_titles_chart.append("Trend Shift (12/26/9)")
                                ...
                                # Bollinger Bands
                                if show_bb:
                                    fig.add_trace(go.Scatter(x=ts, y=display_df['bb_upper'], name="Upper Bound", line=dict(color='rgba(150,150,150,0.5)', width=1), showlegend=False), row=1, col=1)
                                    fig.add_trace(go.Scatter(x=ts, y=display_df['bb_lower'], name="Lower Bound", line=dict(color='rgba(150,150,150,0.5)', width=1), fill='tonexty', fillcolor='rgba(150,150,150,0.06)', showlegend=False), row=1, col=1)
                                    fig.add_trace(go.Scatter(x=ts, y=display_df['bb_mid'], name="Mid Bound", line=dict(color='rgba(180,180,180,0.7)', width=1, dash='dot')), row=1, col=1)

                                # Moving averages
                                ma_config = {
                                    'sma_20':  ('Simple Avg 20',  '#FF9800', show_sma_20),
                                    'sma_50':  ('Simple Avg 50',  '#9C27B0', show_sma_50),
                                    'sma_200': ('Simple Avg 200', '#F44336', show_sma_200),
                                    'ema_12':  ('Recent Avg 12',  '#00BCD4', show_ema_12),
                                    'ema_26':  ('Recent Avg 26',  '#8BC34A', show_ema_26),
                                }
                                ...
                                if show_rsi and rsi_row_num:
                                    fig.add_trace(go.Scatter(x=ts, y=display_df['rsi'], name="Momentum", line=dict(color='#FF9800', width=1.5)), row=rsi_row_num, col=1)
                                ...
                                if show_macd and macd_row_num:
                                    fig.add_trace(go.Scatter(x=ts, y=display_df['macd'], name="Trend Shift", line=dict(color='#2196F3', width=1.5)), row=macd_row_num, col=1)
            ...
            with st.spinner("Loading market context..."):
                try:
                    ...
                    if stock_df.empty:
                        ...
                        with ctx_container:
                            st.subheader(f"{ticker_upper} vs Market Average vs {sector_etf} — {date_display}")
                            st.caption("All series set to 100 at the start of the period — shows relative performance, not raw price.")
                            ...
                            if spy_n is not None:
                                fig.add_trace(go.Scatter(x=spy_n['timestamp'], y=spy_n['indexed'],
                                                         name='Market Average', line=dict(color='#9E9E9E', width=1.5, dash='dot')))
                            ...
                            if spy_ret is not None:
                                m2.metric("Market Average Return", f"{spy_ret:+.2f}%")
                            if alpha_vs_spy is not None:
                                m3.metric("Extra vs Market", f"{alpha_vs_spy:+.2f}%",
                                          help="How much better/worse the stock did vs Market Average")
                            if alpha_vs_sec is not None:
                                m4.metric(f"Extra vs {sector_etf}", f"{alpha_vs_sec:+.2f}%",
